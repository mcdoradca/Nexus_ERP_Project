...
const CampaignDetailsModal = ({ campaign, onClose, onEdit, currentUser, tasks, socket, token, API_URL, fetchData }) => {
  ...
  const handleFinishCampaign = async () => {
     if (!window.confirm("Czy na pewno chcesz zakończyć tę kampanię?")) return;
     try {
        await axios.patch(`${API_URL}/api/campaigns/${campaign.id}`, { status: 'Zakończona' }, {
           headers: { Authorization: `Bearer ${token}` }
        });
        fetchData();
        onClose(); // Zamknij okno i odśwież dane
     } catch (err) {
        console.error("Błąd podczas kończenia kampanii", err);
        alert("Wystąpił błąd podczas kończenia kampanii. Brak uprawnień lub błąd serwera.");
     }
  };

  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-7xl overflow-hidden animate-in zoom-in duration-500 flex flex-col max-h-[85vh] relative min-h-0">
        ...
        <div className="p-10 border-b border-slate-300 flex justify-between items-center bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] shrink-0">
          <div className="flex items-center space-x-6">
            <div className={`w-16 h-16 ${campaign.color?.replace('bg-', 'bg-') || 'bg-slate-900'} rounded-sm flex items-center justify-center shadow-xl shadow-slate-900/10 text-white transition-colors`}>
          ...
          <div className="flex items-center space-x-4">
            {campaign.status !== 'Zakończona' && currentUser && (currentUser.role === 'ADMIN' || currentUser.department === 'MARKETING' || currentUser.department === 'PREZES') && (
              <button 
                onClick={handleFinishCampaign} 
                className="px-6 py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-sm text-[11px] font-black uppercase tracking-widest transition-all shadow-xl shadow-slate-900/30 flex items-center group active:scale-95"
              >
                <CheckCircle2 className="w-4 h-4 mr-3 group-hover:scale-110 transition-transform" /> Zakończ Kampanię
              </button>
            )}
            <button 
              onClick={() => onEdit(campaign)} 
              className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-sm text-[11px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/30 flex items-center group active:scale-95"
            >
              <Edit3 className="w-4 h-4 mr-3 group-hover:scale-110 transition-transform" /> Edytuj Parametry
            </button>
            <button onClick={onClose} className="p-4 hover:bg-slate-900 bg-white border border-slate-400 rounded-sm text-slate-600 hover:text-white transition-all shadow-sm"><X className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-white flex flex-col lg:flex-row min-h-0">
          ...
          <div className="flex-1 p-10 overflow-y-auto custom-scrollbar space-y-12">
            ...
            <div className="grid grid-cols-3 gap-6">
               ...
               <div className="p-6 bg-emerald-50 rounded-sm border border-emerald-100 flex justify-between items-center relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-10">
               ...
               <div className="col-span-2">
                  ...
                  <div className="bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] rounded-sm p-8 border border-slate-300 relative group min-h-[8rem]">
               ...
               <div className="col-span-1">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] flex items-center"><Zap className="w-4 h-4 mr-3" /> Zlecone Zadania</h4>
                  </div>
                  <div className="space-y-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                    {projectTasks.map(t => (
                      <div key={t.id} className="p-5 bg-white border border-slate-300 rounded-sm shadow-sm flex items-center justify-between hover:border-indigo-100 transition-colors cursor-pointer group">
                         <div className="flex items-center space-x-4">
                           <div className={`w-8 h-8 rounded-sm flex items-center justify-center ${t.status==='DONE' ? 'bg-emerald-50 text-emerald-500' : 'bg-slate-100 text-slate-600'} group-hover:scale-110 transition-transform`}>
                    ...
                    {projectTasks.length === 0 && <div className="p-10 text-center text-slate-300 font-black text-[10px] uppercase tracking-widest border-2 border-dashed border-slate-50 rounded-sm">Brak Aktywnych Zadań</div>}
                  </div>
               </div>

               {/* Right Meta Column */}
               <div className="col-span-1 border-l border-slate-300 pl-10 space-y-10">
                 {/* Assignees */}
                 <div>
                   <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mb-6 flex items-center"><Users className="w-4 h-4 mr-3" /> Grupy i Pracownicy</h4>
                   ...
                   <div className="flex flex-wrap gap-2 mb-4">
                     ...
                     {!campaign.assignedGroups?.length && <span className="text-[9px] text-slate-600 uppercase">brak przypisanych działów</span>}
                   </div>

                   <div className="flex flex-wrap gap-3 mt-4">
                      {campaign.assignees?.map(u => (
                        <div key={u.id} className="flex items-center space-x-3 bg-white border border-slate-300 px-3 py-2 rounded-sm shadow-sm">
                          <div title={u.name} className={`w-8 h-8 rounded-sm ${u.color || 'bg-slate-200 text-slate-600'} flex items-center justify-center text-[9px] font-black shadow-inner`}>
                            {u.name?.split(' ').map(n=>n[0]).join('').substring(0,2)}
                          </div>
                          <div>
                            <div className="text-[10px] font-black text-slate-800 uppercase tracking-tight">{u.name}</div>
                            <div className="text-[8px] font-bold text-slate-600 uppercase tracking-widest">{u.department}</div>
                          </div>
                        </div>
                      ))}
                      {!campaign.assignees?.length && <span className="text-[9px] text-slate-600 uppercase mt-2">Brak Liderów</span>}
                   </div>
                 </div>

                 {/* Assets Placeholder -> Real Implementation */}
                 <div>
                   <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mb-6 flex items-center"><Folder className="w-4 h-4 mr-3" /> Baza Plików POSM</h4>
                   ...
                   <div className="space-y-3">
                     {campaign.assets?.map(asset => (
                       <a 
                         ...
                         target="_blank" 
                         rel="noreferrer"
                         className="flex items-center p-4 bg-slate-50 border border-slate-300 rounded-sm hover:bg-white hover:shadow-md transition-all group"
                       >
                         ...
                         <div className="flex-1 min-w-0">
                           ...
                           <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mt-1">Wgrane przez: {asset.uploader?.name}</p>
                         </div>
                       </a>
                     ))}
                     {(!campaign.assets || campaign.assets.length === 0) && (
                       <p className="text-[9px] text-slate-600 uppercase text-center py-4">Brak załączonych plików</p>
                     )}
                 </div>
          ...
          <div className="w-[400px] border-l border-slate-300 bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] flex flex-col min-h-0 shrink-0">
               <UniversalChat 
                 mode="campaign" 
