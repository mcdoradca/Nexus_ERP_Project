    ...
    <div className="flex-1 flex bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] overflow-hidden">
      ...
      <div className={`${isSidebarOpen ? 'w-64 border-r' : 'w-0 border-r-0'} bg-slate-50 border-slate-200 shrink-0 flex flex-col z-30 relative transition-all duration-300 overflow-hidden`}>
         ...
         <div className="p-4 space-y-1.5 flex-1 overflow-y-auto custom-scrollbar">
            ...
            <button 
              onClick={() => handleToolSelect('ITALIAN_FRAME')}
              className={`w-full text-left px-3 py-2.5 rounded-lg text-xs font-medium flex items-center transition-all whitespace-nowrap ${activeSubTool === 'ITALIAN_FRAME' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
            >
              <ImageIcon className={`w-4 h-4 mr-3 shrink-0 ${activeSubTool==='ITALIAN_FRAME'?'text-indigo-600':'text-slate-400'}`} /> Włoska Ramka
            </button>
            <button 
              onClick={() => handleToolSelect('SDS')}
              className={`w-full text-left px-3 py-2.5 rounded-lg text-xs font-medium flex items-center transition-all whitespace-nowrap ${activeSubTool === 'SDS' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
            >
              <CheckCircle2 className={`w-4 h-4 mr-3 shrink-0 ${activeSubTool==='SDS'?'text-indigo-600':'text-slate-400'}`} /> Generator SDS
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
         {activeSubTool === 'SDS' && (
            <div className="flex-1 flex flex-col bg-slate-50 font-sans overflow-hidden animate-in fade-in duration-300">
               <SdsGeneratorTool token={token} API_URL={API_URL} />
            </div>
         )}
         {activeSubTool === 'ITALIAN_FRAME' && (
            <div className="flex-1 flex flex-col bg-slate-50 font-sans overflow-hidden animate-in fade-in duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                           ...
                           <div className="col-span-2 lg:col-span-1 bg-white p-4 rounded-sm border border-slate-300 shadow-sm flex flex-col">
                              ...
                              <div className="space-y-4 text-xs font-black uppercase tracking-widest">
                                 ...
                                 <div className="flex justify-between items-center text-slate-500"><span className="text-slate-600 text-purple-500">Zdjęcia AI (Vision API)</span> <span className="tabular-nums text-purple-600">{(selProd.aiImageCost || 0).toFixed(2)} zł</span></div>
