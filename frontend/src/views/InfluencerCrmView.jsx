...
import { Search, ShieldCheck, Mail, ArrowRight, TrendingUp, X, Loader2, Star, UserPlus, Users, ExternalLink, Edit2, Trash2, Send, MessageSquare, Briefcase, FileText, Check, AlertTriangle } from 'lucide-react';
...
const InfluencerCrmView = ({ currentUser, API_URL, token }) => {
   ...
   const handleGenerateBrief = async () => {
       if (!selectedCreator.deals || selectedCreator.deals.length === 0) {
          alert('Twórca musi być najpierw przypisany do Kampanii (Lejka), by wygenerować Outreach pod produkt.');
          return;
       }
       setIsGeneratingBrief(true);
       try {
           const dealId = selectedCreator.deals[0].id;
           const res = await axios.post(`${API_URL}/api/influencers/deals/${dealId}/outreach`, {}, { headers: { Authorization: `Bearer ${token}` } });
           setGeneratedBrief(res.data.outreachDraft);
       } catch (err) {
           alert("Błąd generowania Outreach: " + (err.response?.data?.error || err.message));
       } finally {
   ...
   return (
      <div className="flex-1 flex flex-col p-10 bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] h-full relative overflow-hidden">
         <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 shrink-0 gap-3">
             <div className="flex items-center">
                 ...
                 <div>
                     <h2 className="text-sm font-bold text-slate-800 leading-tight">Enterprise Influencer CRM</h2>
                     <p className="text-[10px] font-medium text-slate-500">Smart Discovery & Lifecycle Workflows</p>
                 </div>
             </div>
             
             <div className="flex items-center space-x-2 w-full md:w-auto justify-end">
                 <div className="flex flex-1 max-w-lg items-center bg-white border border-slate-200 rounded-md overflow-hidden shadow-sm focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400 transition-all">
                     <span className="pl-3 pr-2 text-indigo-400"><Star className="w-4 h-4 fill-current" /></span>
                     <input 
                         type="text" 
                         value={huntPrompt}
                         onChange={(e) => setHuntPrompt(e.target.value)}
                         placeholder="np. 10 makijażystek do barteru..." 
                         className="py-1.5 px-2 text-xs font-medium text-slate-700 w-full outline-none placeholder:text-slate-400"
                     />
                     <button 
                         onClick={handleHuntInfluencers}
                         disabled={isHunting || !huntPrompt.trim()}
                         className="px-3 py-1.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-l border-indigo-100 text-[10px] font-bold transition-colors flex items-center disabled:opacity-50"
                     >
                        {isHunting ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin"/> Szukam...</> : 'Polowanie AI'}
                     </button>
                 </div>

                 <button onClick={() => setIsSearchModalOpen(true)} className="px-3 py-1.5 bg-slate-900 text-white rounded-md text-[10px] font-medium shadow-sm flex items-center transition-all hover:bg-slate-800">
                    <Search className="w-3.5 h-3.5 mr-1.5 text-indigo-400" /> Wyszukiwarka Vector NLP
                 </button>
             </div>
         </div>

         {/* Widok Repozytorium (Siatka Twórców) - SCHOWANY DO PRZYCISKU */}
         <div className="mb-4 flex-shrink-0 flex items-center justify-between bg-white border border-slate-200 rounded-lg p-3 shadow-sm">
             <div className="flex items-center">
                 <ShieldCheck className="w-5 h-5 mr-3 text-indigo-500" />
                 <div>
                     <h3 className="font-bold text-xs text-slate-800">Katalog Repozytorium (PIM)</h3>
                     <p className="text-[10px] font-medium text-slate-500">Odkrytych twórców: <span className="text-indigo-600 font-bold">{allInfluencers.length}</span></p>
                 </div>
             </div>
             <button 
                 onClick={() => setIsCatalogOpen(true)}
                 className="px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-md text-xs font-medium shadow-sm hover:bg-indigo-100 transition-colors flex items-center"
             >
                Otwórz Katalog 
             </button>
         </div>
         ...
         {isCatalogOpen && (
             <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[9999] flex justify-center items-center">
                 <div className="bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] w-[95vw] h-[90vh] rounded-sm shadow-2xl border border-slate-400 flex flex-col overflow-hidden relative">
                     {/* Header Katalogu */}
                     <div className="px-8 py-6 bg-white border-b border-slate-400 flex justify-between items-center shrink-0">
                         <div>
                         ...
                         <button onClick={() => {setIsCatalogOpen(false); setSelectedCreator(null);}} className="p-3 bg-slate-100 hover:bg-rose-100 hover:text-rose-600 rounded-sm transition-colors">
                             <X className="w-6 h-6" />
                         </button>
                     </div>
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                 {allInfluencers.map(inf => (
                                     <div 
                                        ...
                                        className="bg-white rounded-sm border border-slate-400 overflow-hidden shadow-sm hover:shadow-xl hover:border-indigo-400 transition-all cursor-pointer group flex flex-col"
                                     >
                                         ...
                                         <div className="p-5 flex-1 flex flex-col">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {inf.socialUrl && (
                                                    <a href={inf.socialUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-slate-600 hover:text-indigo-500 transition-colors">
                                     ...
                                     <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            {selectedCreator.socialUrl && (
                                                <a href={selectedCreator.socialUrl} target="_blank" rel="noreferrer" className="flex items-center text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-0.5 rounded-sm transition-colors group">
                                                    Zobacz profil <ExternalLink className="w-3 h-3 ml-1 group-hover:scale-110" />
                                                </a>
                                            )}
                                        </div>
                                     </div>
                                     <span className="text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 px-3 py-1.5 rounded-sm">{selectedCreator.platform}</span>
                                 </div>

                                 <div className="flex border-b border-slate-400 mb-6 shrink-0">
                                     <button onClick={() => {setActiveTab('INFO'); setIsEditing(false);}} className={`flex-1 py-3 justify-center flex items-center text-[10px] font-black uppercase tracking-wider ${activeTab === 'INFO' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-600 hover:text-slate-600'}`}><UserPlus className="w-3 h-3 mr-2"/> Dane</button>
                                     <button onClick={() => setActiveTab('DEALS')} className={`flex-1 py-3 justify-center flex items-center text-[10px] font-black uppercase tracking-wider ${activeTab === 'DEALS' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-600 hover:text-slate-600'}`}><Briefcase className="w-3 h-3 mr-2"/> Kampanie</button>
                                     <button onClick={() => setActiveTab('NOTES')} className={`flex-1 py-3 justify-center flex items-center text-[10px] font-black uppercase tracking-wider ${activeTab === 'NOTES' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-600 hover:text-slate-600'}`}><MessageSquare className="w-3 h-3 mr-2"/> Dziennik</button>
                                 </div>
                                         ...
                                         {!isEditing ? (
                                             <>
                                                 <div className="flex gap-2">
                                                     <button onClick={openEditMode} className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-sm text-[10px] font-black uppercase tracking-wide flex items-center justify-center"><Edit2 className="w-3 h-3 mr-2"/> Edytuj</button>
                                                     <button onClick={handleDeleteCreator} disabled={isDeleting} className="flex-1 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-sm text-[10px] font-black uppercase tracking-wide flex items-center justify-center"><Trash2 className="w-3 h-3 mr-2"/> Usuń</button>
                                                 </div>

                                                 <div className="p-4 bg-slate-50 border border-slate-300 rounded-sm">
                                                     <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">Opis / Nisza Wektorowa</p>
                                                 ...
                                                 <div className="grid grid-cols-2 gap-4">
                                                     <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-sm text-center">
                                                     ...
                                                     <div className="p-4 bg-indigo-50/50 border border-indigo-100 rounded-sm text-center">
                                                 ...
                                                 <div className="p-4 border-2 border-slate-300 rounded-sm">
                                                     <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-3">Widełki Finansowe (Szacowane AI)</p>
                                                     <div className="flex items-center justify-between">
                                                         <div className="text-center">
                                                             <p className="text-[9px] font-bold text-slate-600 uppercase">Minimum</p>
                                                             <p className="text-[15px] font-black text-slate-700">{selectedCreator.minRate || 0} PLN</p>
                                                         </div>
                                                         <div className="w-12 h-px bg-slate-200"></div>
                                                         <div className="text-center">
                                                             <p className="text-[9px] font-bold text-slate-600 uppercase">Maksimum</p>
                                                 ...
                                                 <div className="mt-4">
                                                     {generatedBrief ? (
                                                        <div className="p-4 bg-fuchsia-50 border border-fuchsia-100 rounded-sm relative shadow-inner">
                                                        ...
                                                        <button onClick={handleGenerateBrief} disabled={isGeneratingBrief} className="w-full p-4 border-2 border-fuchsia-100 border-dashed rounded-sm flex items-center justify-center text-fuchsia-500 hover:bg-fuchsia-50 hover:border-fuchsia-200 transition-all disabled:opacity-50 group">
                                             ...
                                             <div className="space-y-3 bg-slate-50 p-4 rounded-sm border border-slate-400 animate-in fade-in zoom-in-95 duration-200">
                                                 ...
                                                 <div className="flex gap-2 mt-2 pt-4 border-t border-slate-400">
                                                     <button onClick={() => setIsEditing(false)} className="flex-1 py-2 text-slate-500 bg-white border border-slate-400 hover:bg-slate-50 rounded-sm text-[10px] font-black uppercase transition-colors">Anuluj</button>
                                                     <button onClick={handleSaveEdit} disabled={isSavingDeal} className="flex-1 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-sm text-[10px] font-black uppercase flex justify-center items-center transition-colors">
                                 ...
                                 {activeTab === 'DEALS' && (
                                     <div className="space-y-4 flex-1">
                                         ...
                                         {(!selectedCreator.deals || selectedCreator.deals.length === 0) ? (
                                             <div className="text-center py-10 opacity-60 bg-slate-50 rounded-sm border border-slate-300">
                                                 <AlertTriangle className="w-8 h-8 text-slate-600 mx-auto mb-2"/>
                                                 <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Brak zaangażowania</p>
                                                 <p className="text-[11px] text-slate-600 mt-1">Ten twórca nie był jeszcze lokowany w projektach.</p>
                                             </div>
                                         ) : (
                                             selectedCreator.deals.map(deal => (
                                                 <div key={deal.id} className="p-4 border border-slate-400 rounded-sm bg-white shadow-sm flex justify-between items-center group hover:border-indigo-300 transition-colors">
                                                     <div>
                                                         <p className="text-[10px] text-indigo-600 font-black uppercase tracking-wider">{deal.campaign?.name || "Luzna Kampania"}</p>
                                                         <p className="text-[11px] font-black text-slate-600 mt-1 bg-slate-100 rounded-sm px-2 py-0.5 inline-block">{deal.status}</p>
                                                     </div>
                                                     <div className="text-right">
                                                         <p className="text-[9px] text-slate-600 uppercase font-black tracking-widest">Stawka Prop.</p>
                                     ...
                                     <div className="flex-1 flex flex-col overflow-hidden h-full">
                                         ...
                                         <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-1 custom-scrollbar min-h-48">
                                             {(!selectedCreator.notes || selectedCreator.notes.length === 0) ? (
                                                 <div className="text-center py-10 opacity-60 bg-slate-50 rounded-sm border border-slate-300 h-full flex flex-col items-center justify-center">
                                                    <MessageSquare className="w-8 h-8 text-slate-300 mb-2" />
                                                    <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Pusty Notatnik</p>
                                                 </div>
                                             ) : (
                                                 selectedCreator.notes.map(note => (
                                                     <div key={note.id} className="bg-white p-4 rounded-sm shadow-sm border border-slate-400 relative group animate-in slide-in-from-bottom-2 fade-in">
                                                         <p className="text-[12px] text-slate-700 font-medium leading-relaxed">{note.content}</p>
                                                         <span className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mt-2 block opacity-70 group-hover:opacity-100 transition-opacity">
                                         ...
                                         <div className="mt-auto shrink-0 pt-2 border-t border-slate-300">
                                             <textarea 
                                                 value={noteContent}
                                                 onChange={(e) => setNoteContent(e.target.value)}
                                                 placeholder="Wiadomość ofertowa, notatka o trudności w negocjacjach..."
                                                 className="w-full text-[12px] font-medium p-3 border border-slate-400 rounded-sm focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 outline-none resize-none h-24 shadow-inner bg-slate-50 focus:bg-white transition-all"
                                             />
                                             <button onClick={handleAddNote} className="w-full mt-2 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-sm text-[11px] font-black uppercase tracking-widest transition-colors flex justify-center items-center shadow border border-indigo-700">
         ...
         <div className="flex-1 overflow-x-auto flex space-x-6 pb-4 custom-scrollbar">
             {columns.map(col => {
                 ...
                 return (
                 <div key={col} className="w-[380px] shrink-0 bg-slate-100/50 border border-slate-400 rounded-sm flex flex-col overflow-hidden shadow-sm">
                     <div className="px-6 py-5 bg-white border-b border-slate-400 flex justify-between items-center z-10 shadow-sm relative">
                         <h3 className="font-black text-[12px] text-slate-800 uppercase tracking-widest flex items-center">
                             <div className="w-2 h-2 rounded-sm bg-emerald-500 mr-2"></div>
                             {col}
                         </h3>
                                ...
                                <div key={deal.id} className="bg-white p-5 rounded-sm border border-slate-400 shadow-sm hover:border-indigo-400 transition-all">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <select 
                                            ...
                                            className="px-3 py-1.5 border border-slate-400 rounded-sm text-[9px] font-black text-slate-600 bg-slate-50 outline-none uppercase cursor-pointer"
                                        >
                                    ...
                                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-200">
                                        <span className="text-[9px] font-black uppercase text-indigo-500">Predictive ROI (EMV)</span>
                                        <span className="text-[10px] font-black text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-sm">
                                            {deal.influencer?.predictiveEngagement ? deal.influencer.predictiveEngagement + ' Zaangażowań' : `~ ${((deal.influencer?.followers || 0) * (deal.influencer?.engagementRate || 0.02) * 0.1).toFixed(0)} PLN`}
                                        </span>
                                    </div>
                                </div>
                            ))
         ...
         {isSearchModalOpen && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                <div className="bg-white rounded-sm shadow-2xl p-8 w-full max-w-2xl border border-slate-400 translate-y-0 opacity-100 transition-all">
                   <div className="flex justify-between items-center mb-8">
                       ...
                       <button onClick={() => setIsSearchModalOpen(false)} className="p-2 bg-slate-100 hover:bg-rose-100 hover:text-rose-600 rounded-sm transition-colors">
                           <X className="w-5 h-5" />
                       </button>
                   </div>
                   ...
                   <div className="relative mb-6">
                      ...
                      <input 
                         ...
                         className="w-full pl-14 pr-4 py-4 bg-slate-50 border-2 border-indigo-100 outline-none rounded-sm font-bold text-[13px] text-slate-700 focus:border-indigo-500 focus:bg-white transition-colors placeholder:text-slate-600"
                      />
                   </div>
                   ...
                   <button 
                      ...
                      className="w-full py-4 bg-slate-900 text-white rounded-sm font-black uppercase tracking-widest text-[11px] shadow-xl hover:bg-indigo-600 transition-colors flex items-center justify-center mb-4 disabled:opacity-50 disabled:hover:bg-slate-900"
                   >
                   ...
                   {!isSearching && searchResults.length > 0 && (
                       <div className="mt-6 border border-slate-400 bg-slate-50/50 rounded-sm p-4 max-h-[300px] overflow-y-auto custom-scrollbar space-y-3">
                          <h4 className="text-[9px] font-black text-slate-600 uppercase tracking-widest px-2 mb-3">Wyniki dopasowania Semantycznego AI (Cosine Similarity)</h4>
                          {searchResults.map((match, idx) => (
                             <div key={idx} className="bg-white p-4 rounded-sm border border-slate-400 shadow-sm flex items-center justify-between group hover:border-indigo-300 transition-colors">
                                <div className="flex items-center space-x-4">
                                   <div className="w-12 h-12 rounded-sm bg-indigo-100 flex items-center justify-center text-indigo-500 font-bold text-lg">
                                ...
                                <div className="flex flex-col items-end">
                                   <div className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-sm text-[13px] border border-emerald-100 font-black mb-2 flex items-center shadow-sm">
                   ...
                   {!isSearching && searchQuery && searchResults.length === 0 && (
                       <div className="p-10 border-2 border-dashed border-slate-400 rounded-sm text-center bg-slate-50/50">
                          <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Naciśnij guzik powyżej aby wykonać uderzenie HTTP do sieci.</p>
                       </div>
                   )}
                </div>
            </div>
