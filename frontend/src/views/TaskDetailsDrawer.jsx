      ...
      <div className="w-full max-w-[1400px] bg-white h-full max-h-[85vh] rounded-sm shadow-[0_30px_100px_rgba(0,0,0,0.4)] flex flex-col animate-in zoom-in duration-500 overflow-hidden relative min-h-0">
        ...
        <div className="p-8 border-b border-slate-300 flex justify-between items-center bg-gradient-to-b from-[#f8fafc] to-[#e2e8f0] shrink-0">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={handleArchive} disabled={isProcessing} className="p-4 bg-white border border-slate-300 hover:border-indigo-300 hover:text-indigo-600 rounded-sm transition-all text-slate-600 shadow-sm disabled:opacity-50" title="Archiwizuj Zlecenie">
               <Archive className="w-6 h-6" />
            </button>
            <button onClick={handleHardDelete} disabled={isProcessing} className="p-4 bg-white border border-slate-300 hover:border-rose-300 hover:text-rose-600 rounded-sm transition-all text-slate-600 shadow-sm disabled:opacity-50" title="Trwale Usuń (Kosz)">
               <Trash2 className="w-6 h-6" />
            </button>
            <div className="w-2"></div>
            <button onClick={onClose} disabled={isProcessing} className="p-4 hover:bg-slate-900 hover:text-white bg-white border border-slate-300 rounded-sm transition-all text-slate-600 shadow-sm disabled:opacity-50" title="Zamknij Szufladę">
               <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row divide-x divide-slate-200">
          ...
          <div className="w-full lg:w-[40%] order-2 lg:order-2 overflow-y-auto custom-scrollbar p-8 space-y-8 bg-blue-50/50 min-w-0 shadow-inner">
              ...
              {isEditingBlocked && (
                <div className="bg-rose-50 p-6 rounded-sm border border-rose-200 space-y-4 animate-in slide-in-from-top-4">
              ...
              {currentTask.isBlocked && !isEditingBlocked && (
                <div className="bg-rose-50 border border-rose-100 rounded-sm p-8 flex items-start space-x-6 shadow-xl shadow-rose-500/5">
             ...
             <div>
               ...
               <div className="mt-4 flex items-center text-[10px] font-black uppercase text-slate-600 tracking-widest">
                  <UserCheck className="w-3 h-3 mr-2" /> Wdrożone przez: {currentTask.creator?.name || 'Automatyzacja Nexus'}
               </div>
             </div>

             {/* Subtasks Section */}
             <div className="bg-slate-50 p-6 rounded-sm border border-slate-400">
                ...
                {isAddingSubtask && (
                  <div className="flex items-center space-x-3 mb-6">
                    <input autoFocus className="flex-1 p-3 bg-white border border-slate-400 rounded-sm outline-none focus:border-indigo-500 text-xs font-bold" placeholder="Tytuł pod-zadania..." value={newSubtaskTitle} onChange={e => setNewSubtaskTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSubtask()} />
                    ...
                    <button onClick={() => setIsAddingSubtask(false)} className="p-3 text-slate-600 hover:text-slate-600"><X className="w-4 h-4" /></button>
                  </div>
                )}
                <div className="space-y-3">
                   {currentTask.subTasks?.map(st => (
                     <div key={st.id} onClick={() => onSelectTask(st)} className="flex items-center justify-between p-4 bg-white border border-slate-300 rounded-sm shadow-sm hover:border-indigo-200 transition-all cursor-pointer group active:scale-[0.98]">
                        <div className="flex items-center">
                          <div className={`w-2 h-2 rounded-sm mr-4 shadow-sm ${st.status === 'DONE' ? 'bg-emerald-500' : st.isBlocked ? 'bg-rose-500' : 'bg-amber-400 group-hover:animate-pulse'}`}></div>
                          <span className={`text-xs font-black uppercase tracking-tight group-hover:text-indigo-600 transition-colors ${st.status === 'DONE' ? 'text-slate-600 line-through' : 'text-slate-800'}`}>{st.title}</span>
                        </div>
                        <span className="text-[9px] font-bold text-slate-600 font-mono group-hover:text-indigo-400">{st.taskId}</span>
                     </div>
                   ))}
          ...
          <div className="w-full lg:w-[25%] order-2 lg:order-1 bg-white p-6 space-y-6 shrink-0 overflow-y-auto custom-scrollbar">
             ...
             <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-400/50 pb-2">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                  {attachments.map(a => (
                     <a key={a.id} href={a.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center p-3 bg-white border border-slate-300 rounded-sm hover:border-indigo-300 hover:shadow-md transition-all group cursor-pointer">
                        ...
                        <div className="flex-1 min-w-0">
                           ...
                           <p className="text-[8px] font-bold text-slate-600">{new Date(a.createdAt).toLocaleDateString()}</p>
                        </div>
                     </a>
                  ))}
                  {attachments.length === 0 && (
                    <div className="px-4 py-6 border-2 border-dashed border-slate-400 rounded-sm flex flex-col items-center justify-center text-center opacity-60">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentTask.activeWorkers.map(w => (
                       <div key={w.id} className={`px-3 py-1.5 rounded-sm ${getDepartmentColor(w.department)} flex items-center text-[9px] font-black tracking-widest animate-in zoom-in`}>
                          <div className="w-1.5 h-1.5 bg-white rounded-sm mr-2 animate-ping"></div> {w.name.split(' ')[0]}
                       </div>
                    ))}
                  </div>
                </div>
             ...
             <div className="space-y-4">
                ...
                {isEditingDates ? (
                  <div className="space-y-3 p-4 bg-white border border-slate-400 rounded-sm shadow-sm">
                    <div><label className="text-[8px] font-black uppercase tracking-widest text-slate-600 block mb-1">Start</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full text-xs font-bold bg-slate-50 p-2 rounded outline-none" /></div>
                    <div><label className="text-[8px] font-black uppercase tracking-widest text-slate-600 block mb-1">Koniec (Deadline)</label><input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full text-xs font-bold bg-slate-50 p-2 rounded outline-none" /></div>
                  </div>
                ) : (
                  <div className="flex flex-col space-y-2">
                    <div className="flex items-center justify-between p-3 bg-white rounded-sm border border-slate-300">
                      <span className="text-[9px] font-black text-slate-600 uppercase">Start</span>
             ...
             <div className="space-y-4">
                ...
                {isEditingGroups && (
                  <div className="p-4 bg-slate-100 rounded-sm flex flex-wrap gap-2">
                     {DEPARTMENTS.map(d => (
                        <button key={d} onClick={() => toggleGroup(d)} className={`px-2 py-1 text-[8px] font-black uppercase tracking-widest rounded border ${currentTask.assignedGroups?.includes(d) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-400 hover:border-indigo-300'}`}>{d}</button>
                     ))}
                  </div>
                )}
             </div>
             ...
             <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {currentTask.assignees?.map(a => (
                    <div key={a.id} className="flex items-center justify-between bg-white px-4 py-2 rounded-sm border border-slate-300 shadow-sm">
                      <div className="flex items-center">
