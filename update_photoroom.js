...
const oldPromptLogic = `    let surface = dict.surfaces[seed % dict.surfaces.length];
    ...
    // Nadpisywanie PIM (przyk³ad nadpisywania dla Slotu 9 lub 8)
    if (tags.includes('detox') && dict.surfaces.some(s => s.includes('black') || s.includes('dark'))) {
        // Przymus ciemnych powierzchni dla Detox/Wêgiel (Slot 8 to Urban Modern, gdzie mamy "dark textured concrete table")
    ...
    if (index === 8) { // Slot 9 - PIM Ingredients
        ...
        if (apiKey) {
            try {
                ...
                console.log("[Photoroom Slot 9] Wyekstrahowano sk³adniki:", ingredients);
            } catch (e) {
                console.error("[Photoroom Slot 9] B³¹d pobierania sk³adników (fallback):", e.message);
            }
        }
