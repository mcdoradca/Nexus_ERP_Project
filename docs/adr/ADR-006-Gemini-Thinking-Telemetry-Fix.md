...
Wykazano to poprzez uruchomienie lokalnego środowiska opartego na natywnym `fetch` uderzającym bezpośrednio w endpoint API REST: `https://generativelanguage.googleapis.com/v1alpha/models/gemini-3.8-flash:generateContent`.
...
Dodatkowo w `nodes.config.js` dla Agenta 11 dodano jawny typ: `thinkingLevel: ThinkingLevel.MEDIUM`, aby ujednolicić konfigurację.
