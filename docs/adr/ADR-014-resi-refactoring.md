# ADR-014: Refaktoryzacja Modułu Resi (Wariant A - Claid API)
...
2. **Claid API**: Zamiast lokalnego modułu AI (rembg), który wyczerpywał zasoby serwera (RAM/CPU), wykorzystano zewnętrzne potężne rozwiązanie Claid API. API to przetwarza zdjęcia, wycina z nich tło i odszumia przy zachowaniu idealnej krawędzi bez spadków jakości.
3. **Sharp do Kompozytowania**: Pobierane z Claid API zdjęcia są natychmiast modyfikowane przy użyciu biblioteki `sharp` w środowisku Node.js. Wymuszane są na nich odpowiednie marginesy (tzw. "padding"), wyśrodkowanie (contain/fit) na formacie pod platformę (np. 1500x1500px) i eksportowane w postaci zoptymalizowanego obrazu JPG/PNG. 
...
3. Testy uciążeniowe ("masywne obciążenie", 50+ plików) powiodły się pomyślnie. Serwer zachował płynność i nie przekroczył pułapu pamięci V8. Wystąpiły jedynie limitowania Rate-Limit od strony dostawcy API (429 Claid), które są łagodzone przez system współbieżności zaimplementowany w `resi.service.js`.
