...
const SIGNAL_WORDS_MAP = {
  "PERICOLO": "NIEBEZPIECZEŃSTWO", "DANGER": "NIEBEZPIECZEŃSTWO",
  "ATTENZIONE": "UWAGA", "WARNING": "UWAGA"
};

const OFFICIAL_CLP_H_PHRASES = {
  H220: "Skrajnie łatwopalny gaz.", H225: "Wysoce łatwopalna ciecz i pary.", H226: "Łatwopalna ciecz i pary.",
...
// ============================================================================
// 8. GŁÓWNY SILNIK PARSERA I KWARANTANNY
// ============================================================================
...
class SDSDocxExporter {
  static async export(sdsData, outPath) {
    ...
    for (let i = 1; i <= 16; i++) {
      ...
      // Jeżeli Sekcja 2 -> Wstaw piktogramy jako ImageRun
        ...
        footers: {
          default: new Footer({
            children: [new Paragraph({
              ...
              children: [
                new TextRun("Zgodnie z Rozporządzeniem (UE) 2020/878 | Strona "),
...
async function runCli() {
  ...
  if (command === "extract") {
    // node sds_processor.js extract plik.pdf payload.json config.json nds.json "NAZWA"
    const [pdfPath, outPath, cfgPath, ndsPath, prodName] = [args[1], args[2], args[3], args[4], args[5]];
    NDSRegistry.loadRegistry(ndsPath);
    const config = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
    
    const engine = new SDSProcessorEngine(config);
    const payload = await engine.prepareAgentPayload(pdfPath, prodName || "PRODUKT");
    fs.writeFileSync(outPath, JSON.stringify(payload, null, 2));
    console.log(`[OK] Pakiet dla Agenta gotowy: ${outPath}`);
    if (payload.ocrDiagnostics) console.log(payload.ocrDiagnostics);

  } else if (command === "build") {
    // node sds_processor.js build payload.json translated.json plik.docx
    const [payloadPath, transPath, outPath] = [args[1], args[2], args[3]];
    const payload = JSON.parse(fs.readFileSync(payloadPath, 'utf8'));
    const translated = JSON.parse(fs.readFileSync(transPath, 'utf8'));
    
    const engine = new SDSProcessorEngine(payload.companyConfig);
    const finalData = engine.mergeCompletedSds(payload, translated);
    await SDSDocxExporter.export(finalData, outPath);
    console.log(`[OK] Zbudowano DOCX (z piktogramami i paginacją): ${outPath}`);
  } else {
    console.log("Użycie: node sds_processor.js [extract|build] <argumenty>");
  }
}
