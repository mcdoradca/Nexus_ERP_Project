...
| `Agent_11_Autofill` | gemini-3.7-flash | Automatyczne wypełnianie parametrów Allegro/PIM. |
| `Agent_Segment_Tone` | gemini-3.1-pro-preview | Adaptacja stylu i tonu opisów do docelowej grupy odbiorczej (Persona). |
| `Agent_Offer_JSON` | gemini-3.7-flash | Mapowanie rozproszonych danych HTML/JSON do pojedynczego, zwartego draftu oferty PIM. |
| `Agent_Photoroom_Prompt` | gemini-3.7-flash | Generowanie promptów do scenerii lifestylowych (Photoroom API) na podstawie parametrów z bazy. |
...
| `Agent_PR_Outreach` | gemini-3.7-flash | Automatyczne generowanie mocno spersonalizowanych wiadomości typu DMs i e-mail pod współpracę barter/płatną. |
| `Agent_Router_SMI` | gemini-3.7-flash | Klasyfikowanie wejść od twórców na poszczególne platformy docelowe podczas planowania postów. |
| `Agent_Facebook_SMI` | gemini-3.7-flash | Generowanie eksperckich draftów/wpisów w formacie specyficznym pod Facebooka. |
| `Agent_Instagram_SMI` | gemini-3.7-flash | Generowanie wpisów dla Instagrama z naciskiem na estetykę i dobór hasztagów. |
| `Agent_TikTok_SMI` | gemini-3.7-flash | Tworzenie dynamicznych zarysów/scenariuszy dla krótkich filmów na platformę TikTok. |
...
| Agent ID | Model | Opis |
|---|---|---|
| `Agent_Logistics_Optimizer` | gemini-3.7-flash | Algorytmiczne kalkulowanie najtańszych gabarytów i podpowiadanie lepszych kartonów. |
| `Agent_Ads_Sentinel` | gemini-3.7-flash | Codzienny zautomatyzowany audyt zwrotu (ROAS) z Allegro Ads i rekomendacje włącz/wyłącz kampanie. |
| `Agent_Data_Analyst` | gemini-3.7-flash | (W Demand Forecast) Szacowanie popytu przy użyciu trendów rynkowych, świąt, i danych twardych (S/A). |
