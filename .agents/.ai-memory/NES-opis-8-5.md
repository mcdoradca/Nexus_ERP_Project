...
4. **Tarcza Formatowania Linków Meet:** Proces akceptacji spotkania generuje wirtualny link w trybie Offline. Aby Google Meet przepuściło kod spotkania przez walidator frontendu, zaimplementowano konwerter heksadecymalny (`meetCodeRaw`), który usuwa łączniki z parametru UUID spotkania, zamienia wszystkie cyfry na przypisane im sztywno litery i grupuje ciąg znaków w bezwzględnie wymagany format `xxx-yyyy-zzz`. Zapobiega to blokadom "Nieprawidłowy kod w adresie URL".
