...
4. **Stały Pokój Rekrutacyjny (Metka Google Meet):** Zrezygnowano ze złożonej, chmurowej autoryzacji Google Cloud na rzecz wysoce zoptymalizowanej logiki "Jednego Pokoju". Moduł `google.meet.service.js` dystrybuuje jeden, stały adres Google Meet (zdefiniowany w pliku `.env` pod zmienną `GOOGLE_MEET_PERMANENT_LINK`), który natychmiastowo zapisuje się do kolumny `meetLink` w bazie Prisma. Ochrona przed spotkaniami nakładającymi się na siebie (tzw. zjawisko Poczekalni) jest gwarantowana na wyższym poziomie abstrakcji – przez sam silnik kalendarza rezerwacji, który twardo odrzuca możliwość zarezerwowania (Double Booking) tego samego slotu czasowego. W ten sposób kandydaci nie wpadają na siebie w pokoju wirtualnym, a system zachowuje zerowe koszty operacyjne przy pełnej stabilności i oficjalnym brandingu Google Meet.
