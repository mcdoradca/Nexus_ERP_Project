...
6. **Agent GEO Text (Copywriter):** (Temp 0.6) Używa wyselekcjonowanego INCI oraz surowego AEO do wygenerowania optymalnego kodu HTML Allegro, zwracając ustrukturyzowany JSON z zagnieżdżonym obiektem `htmlContent` zawierającym 5 sekcji opisu (`opis1` do `opis5`) odpowiadających kafelkom Allegro (z czego moduł 5 to INCI 1:1). Całość obsługiwana jest z pełną wsteczną kompatybilnością (obsługa formatów płaskich i zagnieżdżonych w kontrolerach oraz na froncie).
