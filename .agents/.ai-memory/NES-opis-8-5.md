...
13. **Moduł RMA (Zero-Bleed Hub / Tarcza Anty-Wyłudzeniowa)**
    ...
    * **[Poprawka Architektoniczna] Ochrona Pętli i Tokena:** Token BaseLinkera pobierany jest teraz poprawnie ze zintegrowanej bazy (`SystemSetting`) a nie ulotnego `process.env`. Wdrożono rygorystyczny mechanizm paginacji `getOrderReturns` przy użyciu wbudowanego w API parametru `date_from`, wymuszając przeskakiwanie do przodu (z inkrementacją o 1 sekundę) by uniknąć infinite loop. Zlikwidowano usterkę, przez którą stary kod (z błędnym wariantem parametru) ignorował czas i doprowadzał do pętli nieskończonej przepełniającej Node.js RAM.
