...
4. **Integracja z API Google Meet v2:** Zrezygnowano z generowania wirtualnych kodów w locie. Po stronie serwera wdrożono moduł `google.meet.service.js`, autoryzujący się przez Domain-Wide Delegation, który fizycznie generuje w chmurze Google dedykowane przestrzenie spotkań (`spaces`). Nowo wygenerowany adres URL zwracany przez Google ulega natychmiastowemu i trwałemu zapisowi do kolumny `meetLink` (w nowo zaktualizowanym modelu bazy Prisma `MeetingBooking`). W przypadku braku skonfigurowanych kluczy chmurowych przez Administratora, wdrożono Tarcze Fallback (bezpieczne przekierowanie na publiczny wirtualny pokój Jitsi Meet), aby ochronić potok ERP przed awarią krytyczną.
