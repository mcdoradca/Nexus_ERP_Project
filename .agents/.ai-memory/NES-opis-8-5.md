## Modul: Allegro API Compliance Audit (Poprawki krytyczne)
**Data:** 2026-08-11
**Zmiany:**
...
- **Tarcza Anty-Halucynacyjna AI (Backend Fetcher & Mapping):** Moduł `ai.service.js` wykorzystuje bliźniaczy mechanizm pobierania obrazów (`fetchImageSecure` z wymuszeniem IPv4) przy tworzeniu wsadu dla modelu LLM (Gemini). Zabezpiecza to model przed halucynacjami, które występowały, gdy WAF blokował pobranie pliku. Dodatkowo, po zakończeniu audytu przez Gemini Vision AI, wdrożono **automatyczne mapowanie wejściowych adresów URL** na wyjściową strukturę draftu. Eliminuje to problem halucynowania nazw plików przez model (np. `"zdjecie_1"`, `"zdjecie_2"`) i gwarantuje, że w bazie danych i na frontendzie lądują poprawne linki z serwera BaseLinker.
...
- **Rozszerzenie Slotów Galerii MTool (do 15 zdjęć):** Zwiększono limit analizy zdjęć z galerii BaseLinkera in `ai.service.js` (z limitu 2 do 15 zdjęć w `galleryUrls.slice(0, 15)`). Pozwala to na pełne załadowanie, audyt i poprawne wyświetlanie kompletnych zestawów zdjęć produktów (np. 7 zdjęć dla konkretnego EAN), bez generowania pustych slotów ostrzegawczych w module MTool.
...
6. **Agent GEO Text (Copywriter):** (Temp 0.6) Używa wyselekcjonowanego INCI oraz surowego AEO do wygenerowania optymalnego kodu HTML Allegro, zwracając ustrukturyzowany JSON z zagnieżdżonym obiektem `htmlContent` zawierającym 5 sekcji opisu (`opis1` do `opis5`) według nowej architektury układu blokowego (Obietnica, FAB, Specyfikacja, CTA, INCI).
...
9. **Agent Dopasowania Segmentowego i Tonu (Psycholog Sprzedaży):** (Temp 0.5) Analizuje wygenerowane sekcje opisu i parametry produktu, klasyfikuje go do określonego segmentu rynkowego (kosmetyki, elektronika, dom i ogród, odzież, dziecko) i modyfikuje kontekst oraz ton wypowiedzi z użyciem branżowych triggerów psychologicznych. Ponadto kontroluje czytelność, rozbijając akapity (maks. 3-4 linie) i tworząc logiczną ścieżkę szybkiego skanowania (skimming path) poprzez selektywne pogrubienia <strong>.
...
  
   -   * * K o m p i l a c j a   F r o n t e n d   ( H o t f i x ) * * :   W d r o |o n o   k o m p i l a c j   ( n p m   r u n   b u i l d )   w   [r o d o w i s k u   p o   o s t a t n i c h   p o p r a w k a c h   w   A p p . j s x .   W c z e [n i e j s z e   b r a k i   z r e k o m p i l o w a n i a   k o d u   b y By   p o w o d e m   d l a   k t � r e g o   n a p r a w i o n y   m o d u B  s o c k e t . i o   n i e   p r z y n o s i B  w i d o c z n y c h   z m i a n   u   u |y t k o w n i k a .   W p r o w a d z o n o   d o   U n i v e r s a l C h a t . j s x   b e z p i e c z n i k   d i a g n o s t y c z n y   -   s y s t e m   r z u c i   a l e r t ,   j e [l i   u |y t k o w n i k   s p r � b u j e   z a t w i e r d z i   w i a d o m o [  p r z y   u t r a c o n y m   W e b S o c k e t .  
 \n\n## Aktualizacja - Offer Optimizer V2 (02.08.2026)\n- Wstrzyknięto twarde zmienne bazowe produktu z BaseLinker (gtin_ean, product_name, brand, capacity, line) do obiegów danych agentów AI (agent5Data, agent6Data, agent7Data, agent10Data) w pliku src/modules/offer-optimizer-v2/orchestrator.js. Naprawia to krytyczny błąd halucynowania i opisywania niewłaściwych produktów w module twórczym (Master Copywriter).\n
...
- Zmieniono allowedKeys i dodano logik� parsowania podmiotu w orchestrator.js.
...
### Aktualizacja: R�norodno�� kompozycji (100 preset�w)
- Zgodnie z decyzj� u�ytkownika zamiast sztywnych wariant�w przypisanych do slot�w utworzono pul� dok�adnie 100 sztywnych preset�w kompozycji (od HUGE po HIGH_HORIZON). Zosta�y one wygenerowane na bazie losowych zakres�w i wklejone do kodu (COMPOSITIONS). Zwi�ksza to drastycznie r�norodno�� i redukuje szans� na powt�rzenie takiego samego kadru.

### Aktualizacja: Zintegrowany Beautifier API
- Do zapyta� budowanych przez Chaos Engine w \photoroom.service.js\ dodano sta�y parametr \eautify.mode: ai.auto\. Optymalizuje on produkt wyj�ciowy w zakresie na�wietlenia i ostro�ci przed wyci�ciem t�a i wklejeniem w wygenerowane �rodowisko, bez zwi�kszania koszt�w wywo�ania Image Editing API.
...
- [Frontend/Photoroom V2] Naprawiono b��d 'Brak zdj�cia wej�ciowego', kt�ry wyst�powa� po eksporcie produktu do BaseLinkera, dostosowuj�c PhotographicAuditorCard.jsx do prawid�owej obs�ugi string�w base64 (data:image) w polu originalUrl, zamiast wymagania wy��cznie adres�w URL (http).

- [Photoroom V2] Rozwi�zano problem halucynacji AI zmieniaj�cych sam produkt podczas generowania �rodowiska w slotach 2-9 (Chaos Engine). Zmieniono parametry beautify.mode oraz lighting.mode na 'none', wyci�to negatywny prompt wymuszaj�cy brak logo. Wprowadzono te� limity dla paddingRight/paddingLeft (do max 0.49), naprawiaj�c b��d API Photoroom v2, oraz zap�tlono kompresj� dla sharp, aby rozmiar pliku wyj�ciowego nie przekroczy� 2MB dla BaseLinkera.
...
- Dodano domy�lny tekst (Jeste�my bezpo�rednim importerem...) �adowany automatycznie do pola Modu� 7: Reklama (sekcja7 w editorHtml) w UnifiedProductPipelineView.jsx.

- Poprawka (HOTFIX): Zabezpieczono stan sekcji 7 w UnifiedProductPipelineView przed nadpisywaniem (czyszczeniem) przez event PIPELINE_COMPLETE z WebSocket. Wdro�ono zachowanie istniej�cej zawarto�ci lub domy�lnej w przypadku braku warto�ci w nowo przychodz�cym pakiecie HTML z backendu.

- Aktualizacja (AI Prompty): W�ze� 4 - zamieniono niedozwolon� ikon� ?? na ?? w punktach. W�ze� 6 - zamieniono ikon� tarczy ??? na strza�k� ?? w ostrze�eniach CLP/GPSR, z uwagi na blokady po stronie Allegro.

- Aktualizacja (AI Prompty): Spolszczenie struktury AEO - zamieniono s�owo 'Answer' na 'Odpowied�' w W�z�ach 4, 6 i 7. W�ze� 4 - zamieniono usuni�t� kropl� na zielonego ptaszka ?.
...
### Nazwa operacji/zadania: Poprawka Photoroom AI (offer-optimizer-v2)
...
- Agentowi 11 (Prompt Master) dodano jawny parametr thinkingLevel: ThinkingLevel.MEDIUM w nodes.config.js.
