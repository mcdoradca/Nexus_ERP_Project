...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    } catch (err) { alert('B┼é─ůd logowania'); }
  };
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      ...
      fetchData();
    } catch (err) { alert('B┼é─ůd tworzenia zadania'); }
  };
  ...
  const handleCreateBrand = async (e) => {
    ...
    } catch (err) { alert('B┼é─ůd tworzenia marki'); }
  };
  ...
  const handleCreateProduct = async (e) => {
    ...
    } catch (err) { alert('B┼é─ůd tworzenia produktu'); }
  };
    ...
    <div className="flex-1 flex flex-col p-10 bg-[#f8fafc] min-h-0 overflow-hidden">
      <div className="flex items-center justify-between mb-10 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 font-bold uppercase tracking-widest mt-1">Zarz─ůdzanie Jednostkami i Zadaniami Zespo┼éu</p>
        </div>
            ...
            <div key={p.id} onClick={() => setSelectedProject(p)} className="group bg-white rounded-[3rem] border border-slate-100 p-10 shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_25px_60px_rgba(0,0,0,0.1)] hover:border-indigo-200 transition-all cursor-pointer relative overflow-hidden flex flex-col active:scale-[0.98]">
              ...
              <div className="mt-auto">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Post─Öp Realizacji</span>
          ...
          return (
            <div key={c.id} className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_40px_80px_rgba(0,0,0,0.1)] transition-all relative overflow-hidden flex flex-col lg:flex-row lg:items-center justify-between gap-10 group">
              ...
              <div className="flex items-center space-x-12 shrink-0 bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100">
                ...
                <div className="text-center">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Post─Öp</div>
    ...
    <div className="flex-1 flex h-full min-h-0 overflow-hidden bg-white relative">
      <div className="w-96 border-r border-slate-100 flex flex-col shrink-0 bg-[#f8fafc]">
        <div className="p-10 border-b border-slate-100 bg-white/50 backdrop-blur-md sticky top-0 z-10">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.3em] mb-6">Wiadomo┼Ťci i Kana┼éy</h3>
          <div className="relative group">
            ...
            <input className="w-full pl-14 pr-6 py-4 bg-white border border-slate-200 rounded-2xl text-xs outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold placeholder:text-slate-400" placeholder="Szukaj os├│b..."/>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-3">
          <div onClick={() => setActiveChat('general')} className={`p-6 rounded-[2rem] cursor-pointer transition-all flex items-center justify-between group ${activeChat === 'general' ? 'bg-indigo-600 shadow-2xl shadow-indigo-600/20 text-white' : 'bg-white border border-slate-100 hover:border-indigo-200'}`}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-[11px] font-black uppercase tracking-wider"># Kana┼é Og├│lny</div>
                <div className={`text-[9px] font-bold mt-1 ${activeChat === 'general' ? 'text-indigo-200' : 'text-slate-400'}`}>Og┼éoszenia firmowe</div>
              </div>
            </div>
          ...
          <div className="pt-8 px-4 pb-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Cz┼éonkowie Zespo┼éu</div>
      ...
      <div className="flex-1 flex flex-col bg-white min-w-0">
        <div className="h-28 border-b border-slate-100 flex items-center justify-between px-10 bg-white/50 backdrop-blur-xl shrink-0 z-10">
          <div className="flex items-center">
            ...
            <div className="ml-6">
              <h3 className="text-lg font-black text-slate-900 uppercase tracking-widest leading-none">{activeChat === 'general' ? 'Strumie┼ä Og├│lny' : users.find(u => u.id === activeChat)?.name}</h3>
              <div className="flex items-center mt-2.5">
                <span className="w-2 h-2 bg-emerald-500 rounded-full mr-3 shadow-lg shadow-emerald-500/20 animate-pulse"></span>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{activeChat === 'general' ? 'Otwarta dyskusja strategiczna' : 'Bezpo┼Ťredni kana┼é szyfrowany'}</span>
              </div>
            </div>
          </div>
          <div className="flex -space-x-3">
        ...
        <div className="p-10 bg-white border-t border-slate-100 shrink-0 z-10">
          <form onSubmit={(e) => { e.preventDefault(); if(!newChatMessage.trim()) return; /* sendMessage logic would normally go here if implemented in handler */ }} className="flex items-center space-x-4 bg-slate-100/50 p-3 rounded-[2.5rem] border border-slate-200 focus-within:ring-8 focus-within:ring-indigo-500/5 transition-all">
            ...
            <input value={newChatMessage} onChange={(e) => setNewChatMessage(e.target.value)} placeholder="Napisz do zespo┼éu..." className="flex-1 bg-transparent border-none outline-none text-sm font-bold py-2 px-4 placeholder:text-slate-400"/>
    ...
    <div className="flex-1 flex flex-col p-12 bg-[#f8fafc] min-h-0 overflow-hidden">
      <div className="flex items-center justify-between mb-12 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 font-bold uppercase tracking-widest mt-2">Zarz─ůdzanie infrastruktur─ů Nexus ERP</p>
        </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] sticky top-0 z-10 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-10 py-6 text-right">Zarz─ůdzanie</th>
                </tr>
              </thead>
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-10 relative overflow-hidden">
      ...
      <div className="w-full max-w-xl bg-white/5 backdrop-blur-3xl rounded-[4rem] shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden p-16 border border-white/10 relative z-10">
        ...
        <form onSubmit={handleLogin} className="space-y-8">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] block ml-4">Has┼éo Dost─Öpowe</label>
            <div className="relative group">
              ...
              <input type="password" required placeholder="ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó" className="w-full pl-16 pr-8 py-5 bg-white/5 border border-white/10 rounded-3xl outline-none focus:ring-4 focus:ring-indigo-500/20 text-white font-bold transition-all placeholder:text-slate-600" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} />
            </div>
          </div>

          <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-6 rounded-3xl transition-all shadow-2xl shadow-indigo-600/30 uppercase tracking-[0.2em] text-sm mt-4 active:scale-95">Inicjalizuj Sesj─Ö</button>
        </form>

        <div className="mt-16 text-center border-t border-white/5 pt-8">
          <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest">Prywatne ┼Ürodowisko APS Workspace &copy; 2026</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex-1 flex flex-col p-10 bg-[#f8fafc] h-full w-full relative min-h-0 overflow-hidden">
            ...
            {isAdmin && (
              <>
                <button onClick={() => setIsNewBrandModalOpen(true)} className="px-6 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 shadow-sm flex items-center group transition-all">
                  <Target className="w-4 h-4 mr-3 text-indigo-500 group-hover:scale-110 transition-transform" /> Dodaj Mark─Ö
                </button>
        ...
        <div className="bg-white rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] border border-slate-200/50 flex-1 flex flex-col min-h-0 overflow-hidden relative">
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-slate-50/80 sticky top-0 z-10 border-b border-slate-100 backdrop-blur-xl">
                <tr>
                  ...
                  {isAdmin && (
                    <>
                      <th className="p-8 font-black uppercase tracking-[0.2em] text-[10px] text-slate-400">Struktura Koszt├│w</th>
                        ...
                        <>
                          <td className="p-8">
                            <div className="flex items-center justify-between text-[10px] mb-1">
                              ...
                              <span className="font-bold text-slate-700 tabular-nums">{p.basePrice.toFixed(2)} z┼é</span>
                            </div>
                            <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden mb-2">
                               <div className="h-full bg-slate-400" style={{ width: `${(tc/p.salePrice)*100}%` }}></div>
                            </div>
                            <div className="text-[11px] font-black text-slate-900 tabular-nums">TC: {tc.toFixed(2)} z┼é</div>
                          </td>
          ...
          <div className="p-8 bg-slate-50/50 border-t border-slate-100 shrink-0 flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">┼ü─ůcznie produkt├│w: {products.length}</span>
            <div className="flex space-x-2">
               ...
               <button className="px-5 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">1 ÔÇô 10</button>
  ...
  const renderKanban = () => {
    const columns = [
      { id: 'TODO', name: 'Zaleg┼ée / Backlog', color: 'bg-slate-100', icon: <Clock className="w-3 h-3 mr-2" />, dot: 'bg-slate-400' },
      ...
      { id: 'DONE', name: 'Zako┼äczone', color: 'bg-emerald-50', icon: <CheckCircle2 className="w-3 h-3 mr-2 text-emerald-600" />, dot: 'bg-emerald-600' }
    ];
        ...
        <div className="w-full max-w-[50rem] bg-white h-full shadow-[-40px_0_100px_rgba(0,0,0,0.2)] flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-12 space-y-12 bg-white">
            ...
            <div className="grid grid-cols-3 gap-8">
               <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                  ...
                  <div className="text-2xl font-black text-slate-900">{projectTasks.length} <span className="text-slate-400 text-sm">Zada┼ä</span></div>
               </div>
               <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Uko┼äczono</div>
                  <div className="text-2xl font-black text-slate-900">{doneTasks} <span className="text-slate-400 text-sm">Task├│w</span></div>
               </div>
               <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Post─Öp</div>
            ...
            <div>
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 flex items-center">
                <Layout className="w-4 h-4 mr-3" /> Brief i Za┼éo┼╝enia
              </h4>
            ...
            <div>
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8 flex items-center">
                <Users className="w-4 h-4 mr-3" /> Zesp├│┼é Dedykowany
              </h4>
              <div className="flex flex-wrap gap-4">
                ...
                )) : <div className="text-[11px] font-black text-slate-300 uppercase tracking-widest py-4">Brak przypisanych os├│b</div>}
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] flex items-center">
                  <Hash className="w-4 h-4 mr-3" /> Rejestr Zada┼ä ({projectTasks.length})
                </h4>
              ...
              <div className="space-y-4">
                ...
                {projectTasks.length === 0 && <div className="py-20 text-center text-slate-300 font-black text-[10px] uppercase tracking-widest border-2 border-dashed border-slate-50 rounded-[2rem]">Projekt nie ma jescze zada┼ä</div>}
              </div>
            </div>
          </div>
          ...
          <div className="p-10 bg-slate-50 border-t border-slate-100 shrink-0">
             <div className="flex items-center justify-between">
                ...
                <button className="px-10 py-5 bg-slate-900 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 hover:scale-105 active:scale-95 transition-all shadow-2xl">Zako┼äcz Projekt</button>
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-5xl bg-white h-full max-h-[55rem] rounded-[4rem] shadow-[0_50px_150px_rgba(0,0,0,0.3)] flex flex-col animate-in zoom-in duration-500 overflow-hidden relative">
          ...
          <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
                 ...
                 <div className="space-y-10">
                    <div className="flex space-x-6 animate-in slide-in-from-left duration-300">
                       ...
                       <div className="flex-1">
                          ...
                          <div className="bg-slate-50 p-6 rounded-[2rem] rounded-tl-none border border-slate-100 text-[13px] font-bold text-slate-600 leading-relaxed">System zsynchronizowa┼é to zadanie z tablic─ů Nexus 2.0. Status ustawiony na domy┼Ťlny.</div>
                       </div>
                    </div>
                    {/* Placeholder for real comments */}
                    <div className="text-center py-16 text-slate-300 font-black text-[10px] uppercase tracking-widest border-2 border-dashed border-slate-50 rounded-[3rem]">Brak nowych komentarzy w w─ůtku</div>
                 </div>
               </div>
            </div>
            ...
            <div className="w-full lg:w-96 bg-[#f8fafc] border-l border-slate-100 p-12 space-y-10 shrink-0">
               <div className="space-y-3">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Jednostka Nadrz─Ödna</h4>
                  <div className="flex items-center bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm hover:border-indigo-200 transition-colors">
                     ...
                     <span className="text-xs font-black text-slate-800 uppercase truncate">{selectedTask.project?.name || selectedTask.campaign?.name || 'Operacja Wolnostoj─ůca'}</span>
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {(!selectedTask.assignees || selectedTask.assignees.length === 0) && <div className="text-[10px] font-black text-slate-300 uppercase py-4">Brak operator├│w</div>}
                  </div>
                  <button className="w-full py-4 text-[10px] font-black text-indigo-600 hover:bg-white rounded-2xl border border-dashed border-indigo-200 uppercase tracking-widest transition-all mt-4">+ Przypisz Osob─Ö</button>
               </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[120] flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.3)] w-full max-w-2xl overflow-hidden animate-in zoom-in duration-500 flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleCreateTask} className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
                <div>
                  <label className={labelClass}>Tytu┼é Operacyjny</label>
                ...
                <div>
                  <label className={labelClass}>Szczeg├│┼éowy Opis Techniczny</label>
                ...
                <button type="submit" className="w-full py-6 bg-slate-900 text-white font-black rounded-3xl shadow-2xl hover:bg-indigo-600 hover:scale-[1.02] active:scale-[0.98] transition-all uppercase tracking-[0.2em] text-sm mt-4">Uruchom Przep┼éyw Zadania</button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[120] flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-500">
              ...
              <form onSubmit={handleCreateBrand} className="p-10 space-y-8">
                ...
                <button type="submit" className="w-full py-6 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-3xl shadow-2xl shadow-indigo-600/20 transition-all uppercase tracking-[0.2em] text-sm active:scale-95">Zatwierd┼║ w Bazie PIM</button>
              </form>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-[4rem] shadow-[0_50px_150px_rgba(0,0,0,0.4)] w-full max-w-4xl overflow-hidden animate-in zoom-in duration-700 max-h-[90vh] flex flex-col relative">
              <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-[#f8fafc] shrink-0">
                <div className="flex items-center">
                   ...
                   <div>
                     ...
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Inicjalizacja Modu┼éu PIM & Unit Economics</p>
                   </div>
                </div>
              ...
              <form onSubmit={handleCreateProduct} className="p-12 space-y-12 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 gap-10">
                  ...
                  <div>
                    <label className={labelClass}>SKU (Identyfikator Wewn─Ötrzny) *</label>
                  ...
                  <div>
                    <label className={labelClass}>Przynale┼╝no┼Ť─ç Brandowa *</label>
                    <select required className={inputClass} value={newProductForm.brandId} onChange={e => setNewProductForm({...newProductForm, brandId: e.target.value})}>
                      <option value="">Wybierz mark─Ö z listy...</option>
                      {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className={labelClass}>Stan Magazynowy Pocz─ůtkowy</label>
                ...
                <div className="pt-12 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-3 gap-8">
                    ...
                    <div><label className={labelClass}>Transport In (c┼éa)</label><input type="number" step="0.01" className={inputClass} value={newProductForm.inboundTransportCost} onChange={e => setNewProductForm({...newProductForm, inboundTransportCost: e.target.value})} /></div>
                    ...
                    <div><label className={labelClass}>BDO / ┼Ümieci</label><input type="number" step="0.01" className={inputClass} value={newProductForm.bdoEprCost} onChange={e => setNewProductForm({...newProductForm, bdoEprCost: e.target.value})} /></div>
                    ...
                    <div>
                      <label className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em] mb-3 block ml-2">Cena Sprzeda┼╝y Detalicznej *</label>
        ...
        <header className="h-24 sticky top-0 flex items-center justify-between px-10 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 z-50">
          <div className="flex items-center space-x-6">
            <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">
              {activeTab === 'kanban' && 'Tablica Zada┼ä'}
              {activeTab === 'campaigns' && 'Kampanie Marketingowe'}
              {activeTab === 'projects' && 'Zarz─ůdzanie Projektami'}
              {activeTab === 'products' && 'Katalog Produkt├│w SKU'}
            ...
            {showNotifications && (
              <div className="absolute right-10 top-24 w-96 bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100 z-[100] overflow-hidden">
                ...
                <div className="max-h-[30rem] overflow-y-auto custom-scrollbar">
                  {notifications.length === 0 ? <div className="p-10 text-center text-slate-400 font-bold text-xs italic">Brak nowych powiadomie┼ä</div> : notifications.map(n => (
