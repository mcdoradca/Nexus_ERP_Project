...
Format wyjĹ›ciowy: ZwykĹ‚y tekst.`;
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_OSINT", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
...
Odpowiedz w postaci zwiÄ™zĹ‚ego, czystego tekstu w jÄ™zyku polskim.`;
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_Sentiment", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
...
async function generateNativeAnalysis(textContent, nativeImagesUrls = [], analysisMode = "STANDARD") {
    ...
    try {
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_VisionNative", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
...
`;
    ...
    try {
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_OfferJSON", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
...
async function auditOfferImages(primaryImageUrl, galleryUrls = []) {
    ...
    try {
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_ImageAudit", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
...
`;
    ...
    try {
        ...
        if (result && result.response && result.response.usageMetadata) { await AiMetricsService.logUsage("Legacy_11_Autofill", "gemini-3.8-flash", result.response.usageMetadata, true, 1); }
        let text = result.response.text();
