...
# rozmów. Zastępuje i rozszerza HANDOFF_SESJA_2.md. W razie sprzeczności między
...
- GIT, commit c49c3ea "freeze: stan przed resetem...", gałąź
  ...
  JEDYNE zmienione pliki kodu: ai.service.js i compile_sot.js (drobne zmiany);
  ...
  wygenerował: audyt_sesja1_ai_service.diff, audyt_sesja1_compile_sot.diff).
...
- Baseline telemetrii (kokpit, przed zmianami) — plik BASELINE_TELEMETRIA.md.
...
- Reguły HTML Allegro wg RAG_SOT_01 (jedyne źródło prawdy): dozwolone TYLKO
  h1,h2,p,ul,ol,li,b. ZAKAZ <br> (akapity przez osobne <p>), ZAKAZ <strong>,
  ZAKAZ <b> wewnątrz nagłówków. SHARED_RULES_v4.1 jest z tym zgodny;
  SHARED_RULES_v4.md (bez .1) jest BŁĘDNY i nieobowiązujący.
...
- W bazie telemetrii istnieją agentId spoza kanonicznej listy (m.in.
  Agent_3_SEOTitle, Agent_Title, Agent_AEO, Test_Agent...) — dowód, że stary
  moduł wykonywał wywołania LLM poza architekturą kanoniczną.

### 3B. NIEZWERYFIKOWANE (raporty sesji 1 — traktuj jako HIPOTEZY):
- Tabela audytu 22 wywołań LLM (AUDYT_LLM_OFFER_OPTIMIZER.md), TRACE ścieżki
  analyzeSingle → generateNativeAnalysis, klasyfikacje reliktów, stringi
  modeli per węzeł, twierdzenie o wdrożonych logowaniach Legacy_*.
...
OP-2: Stary moduł offer-optimizer = kwarantanna/code freeze. Jedyny
  dopuszczalny wyjątek NA OSOBNĄ KOMENDĘ: flaga env FAST_PATH_CHEMISTRY_GUARD
  (produkty sds_required==true lub kategorie chemiczne/biobójcze na starych
...
OP-6: Migracja bazy (RAG v2): wyłącznie addytywna (ADD COLUMN nullable/
  default), backup przed, `npx prisma migrate dev --name rag_v2_metadata`
  (nie db push), dopiero w etapie E3.
OP-7: C2PA: bez instalowania bibliotek — stub zwracający
  C2PA_CHECK_UNAVAILABLE traktowany jako OSTRZEŻENIE, nigdy jako "metadane OK".
...
S-1: Zwroty H/P, hasła ostrzegawcze (NIEBEZPIECZEŃSTWO/UWAGA), kod UFI,
  ...
  SHA-256 i NIE przechodzą przez żaden model generatywny; verify_frozen przed
...
S-2: Bramki składnikowe zatrzymują potok (nie ostrzegają): GATE-1 substancje
  ...
  INGREDIENT_NOT_COSMETIC (SOT 06 §2 — ketokonazol, hydrochinon, tretinoina,
  antybiotyki, kortykosteroidy, EGF/FGF; firma NIE handluje lekami),
  GATE-3 składnik nieznany = UNKNOWN_INGREDIENT_NEEDS_LOOKUP → pomiń w opisie,
  zakaz zgadywania.
S-3: Brak karty SDS przy sds_required==true = twarde zatrzymanie potoku.
...
S-5: Reguły prawne i czarne listy NIGDY przez similarity search — wyłącznie
  deterministyczne prefiksy statyczne (RAG_ORCHESTRATION §0). RAG służy
  tylko słownikom składnikowym.
S-6: Listy zakazanych słów/substancji kopiowane 1:1 z SHARED_RULES_v4.1 —
...
S-8: Zakaz dark patterns (fałszywa pilność, profilowanie lękowe); Pratfall
  ...
  nie są claimami o produkcie bez dowodu w PIM (SHARED_RULES §J).
...
E0 — KONTRAKTY + WERYFIKACJA API (read-only)
  ...
  potwierdź czysty punkt startu; (2) KONTRAKTY_V2.md: endpointy HTTP
  ...
  (wejście do decyzji o OP-2/bezpieczniku); (3) WERYFIKACJA_API_V2.md:
...
E1 — SZKIELET V2
  ...
  SHARED_RULES v4.1]+[dane SKU na końcu], bez cache (OP-1); prompty ładowane
...
E2 — WALIDATORY KODOWE I BRAMKI (zero LLM)
  Zadania wg 00_PLAN §2 + SHARED_RULES v4.1: ean_checksum, route_chemical
  (A4 wywoływany TYLKO dla chemii), scan_stopwords (§A), leksykon medyczny
  (§D), validate_html_whitelist (§B: tylko h1,h2,p,ul,ol,li,b; bez br/strong;
  bez b w nagłówkach), diff_numeric PIM↔HTML, emoji_structure_check (§C),
  bramki GATE-1/2/3 (§I), c2pa stub (OP-7), freeze/verify_frozen (S-1).
...
E3 — WARSTWA RAG V2
  ...
  metadanymi wg RAG_ORCHESTRATION §1; getKnowledgeForIngredients w
...
E4 — PEŁNY POTOK A1–A10
  Zadania: orkiestrator-kod (maszyna stanowa wg Agent_0_prompt_v4.md);
  sekwencja faz; A7 w trybie diff (wejście/wyjście tylko s1/s2/s4);
  A10 w trybie semantycznym z repair_patches; pętla rewizyjna per sekcja,
